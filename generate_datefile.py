...
def parse_args():
    ...
    # 添加参数，解析输出的数据库文件路径
    parser.add_argument('--output_db', required=True, type=str, help='Output database file path')
...
def create_table(conn: sqlite3.Connection, table_name: str, columns: List[str]):
    cursor = conn.cursor()
    sql = "Create Table if not exists {} ({})".format(table_name, ", ".join([f'{col} TEXT' for col in columns]))
    print("\t",sql)
    cursor.execute(sql)
    conn.commit()
    cursor.close()
...
if __name__ == '__main__':
    ...
    for file_name, file_path in scan_csv_files(args.csv_folder).items():
        ...
        print("Creating and Writing data to table: {} ({})".format(table_name, file_name))
